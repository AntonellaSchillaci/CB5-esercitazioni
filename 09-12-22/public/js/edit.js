const GET = async (cognome) => {
  const url = `http://localhost:3000/attore?cognome=${cognome}`;
  const res = await fetch(url);
  return await res.json();
};
function getActor(cognome) {
  const url = `http://localhost:3000/attore_cognome?cognome=${cognome}`;
  const GET = async (url) => {
    const res = await fetch(url);
    return await res.json();
  };
...
search_btn.addEventListener("click", (e) => {
  e.preventDefault();
  GET(cognomeEl.value).then((attore) => {
